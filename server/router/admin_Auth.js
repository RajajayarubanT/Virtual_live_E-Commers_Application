...
const Admin_userSchema = mongoose.model("Admin_users");
const crypto = require('crypto')
const bcrypt = require("bcryptjs");
const jwt = require('jsonwebtoken');
const { JWT_SECRET, MONGOURI, JWT_Admin_SECRET } = require("../config/key")
...
const Customer_userSchema = mongoose.model("users");
const Customer_OrderDatas_Schema = mongoose.model("Customer_OrderDatas");
//e-mail verification
...
router.post('/admin_signup', (req, res) => {
    ...
    Admin_userSchema.findOne({ email: email })
        .then((savedUser) => {
            if (savedUser) {
            ...
            bcrypt.hash(password, 12)
                .then(hashedpassword => {
                    const user = new Admin_userSchema({
                        name: name,
                        email: email,
...
router.post('/reset-password', (req, res) => {
    crypto.randomBytes(32, (err, buffer) => {
        ...
        Admin_userSchema.findOne({ email: req.body.email })
            .then(user => {
                if (!user) {
...
router.post('/new-password', (req, res) => {
    ...
    Admin_userSchema.findOne({ resetToken: sentToken, expireToken: { $gt: Date.now() } })
        .then(user => {
            if (!user) {
...
//Employee Count
router.get("/requested-employeesData", (req, res) => {
    Employee_userSchema.find()
        .then((employee) => {
            Customer_userSchema.find().then((customer) => {
                Customer_OrderDatas_Schema.find().then((customerOrder) => {
                    res.json({ employee, customer, customerOrder });
                });
            });
        })
        .catch((err) => {
